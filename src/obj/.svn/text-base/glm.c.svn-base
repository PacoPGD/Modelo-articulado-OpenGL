...
glmFindMaterial(GLMmodel* model, char* name)
{
    ...
    /* didn't find the name, so print a warning and return the default
       material (0). */
    __glmWarning((char*)"glmFindMaterial():  can't find material \"%s\".", name);
    i = 0;
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    if (!file) {
        __glmFatalError((char*) "glmReadMTL() failed: can't open material file \"%s\".",
			 filename);
    }
    free(filename);
    ...
    while(fscanf(file, "%s", buf) != EOF) {
		...
		__glmFatalError((char*)"glmReadMTL: Got \"%s\" instead of \"newmtl\" in file \"%s\"", buf, filename);
            fgets(buf, sizeof(buf), file);
            nummaterials++;
        ...
        case 'N':
            ...
            case 'i':
                /* Refraction index.  Values range from 1 upwards. A value
                   ...
                   refraction. */
                __glmWarning((char*)"refraction index ignored");
                fgets(buf, sizeof(buf), file);
                break;
            default:
		__glmWarning((char*)"glmReadMTL: Command \"%s\" ignored", buf);
                fgets(buf, sizeof(buf), file);
                break;
            }
	    break;
	...
	break;
		...
		__glmFatalError((char*)"glmReadMTL: Got \"%s\" instead of \"illum\" in file \"%s\"", buf, filename);
		...
		if(illum != 2)	/* illum=2 is standard lighting */
		    __glmWarning((char*)"illum material ignored: illum %d", illum);
	    }
	    break;
        case 'm':
            /* texture map */
            ...
            if(strncmp(buf, "map_Kd", 6) == 0) {
                ...
                __glmWarning((char*)"map %s %s ignored",buf,t_filename);
                free(t_filename);
                fgets(buf, sizeof(buf), file);
            }
            break;
	    ...
	    fgets(buf, sizeof(buf), file);
            __glmWarning((char*)"reflection type ignored: r%s",buf);
	    break;
	default:
	    /* eat up rest of line */
	    fgets(buf, sizeof(buf), file);
...
glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname)
{
    ...
    if (!file) {
        __glmFatalError( (char*)"glmWriteMTL() failed: can't open file \"%s\".",
			 filename);
    }
    free(filename);
...
glmFirstPass(GLMmodel* model, FILE* file) 
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
            ...
            case 't':           /* texcoord */
                ...
                break;
            default:
                __glmFatalError((char*)"glmFirstPass(): Unknown token \"%s\".", buf);
                break;
            }
            break;
	case 'm':
	    if(strncmp(buf, "mtllib", 6) != 0)
		__glmFatalError((char*)"glmReadOBJ: Got \"%s\" instead of \"mtllib\"", buf);
	    fgets(buf, sizeof(buf), file);
	    sscanf(buf, "%s %s", buf, buf);
	...
	case 'u':
		...
		__glmFatalError((char*)"glmReadOBJ: Got \"%s\" instead of \"usemtl\"", buf);
	    /* eat up rest of line */
	    fgets(buf, sizeof(buf), file);
	    break;
	case 'g':               /* group */
...
glmVertexNormals(GLMmodel* model, GLfloat angle, GLboolean keep_existing)
{
	...
	/* calculate an average normal for this vertex by averaging the
	   facet normal of every triangle this vertex is in */
        node = members[i];
        if (!node)
            __glmWarning( (char*)"glmVertexNormals(): vertex %d w/o a triangle", i);
	...
	int avg_index = -1;
		...
		while (model->numnormals < numnormals) {
		    __glmWarning((char*) "glmVertexNormals(): realloc %d+100\n", model->numnormals+100);
...
glmReadOBJ(char* filename)
{
    ...
    if (!file) {
	return NULL;
        __glmFatalError( (char*)"glmReadOBJ() failed: can't open data file \"%s\".",
			 filename);
    }
...
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode)
{
    ...
    if (mode & GLM_FLAT && !model->facetnorms) {
        __glmWarning((char*)"glmWriteOBJ() warning: flat normal output requested "
		     "with no facet normals defined.");
        mode &= ~GLM_FLAT;
    }
    if (mode & GLM_SMOOTH && !model->normals) {
        __glmWarning((char*)"glmWriteOBJ() warning: smooth normal output requested "
		     "with no normals defined.");
        mode &= ~GLM_SMOOTH;
    }
    if (mode & GLM_TEXTURE && !model->texcoords) {
        __glmWarning((char*)"glmWriteOBJ() warning: texture coordinate output requested "
    ...
    if (mode & GLM_COLOR && !model->materials) {
        __glmWarning((char*)"glmWriteOBJ() warning: color output requested "
		     "with no colors (materials) defined.");
        mode &= ~GLM_COLOR;
    }
    if (mode & GLM_MATERIAL && !model->materials) {
        __glmWarning((char*)"glmWriteOBJ() warning: material output requested "
		     "with no materials defined.");
        mode &= ~GLM_MATERIAL;
    }
    if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
        __glmWarning((char*)"glmWriteOBJ() warning: color and material output requested "
    ...
    file = fopen(filename, "w");
    if (!file) {
        __glmFatalError((char*) "glmWriteOBJ() failed: can't open file \"%s\" to write.",
			 filename);
    }
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    /* do a bit of warning */
    if (mode & GLM_FLAT && !model->facetnorms) {
        __glmWarning((char*)"glmDraw() warning: flat render mode requested "
		     "with no facet normals defined.");
        mode &= ~GLM_FLAT;
    }
    if (mode & GLM_SMOOTH && !model->normals) {
        __glmWarning((char*)"glmDraw() warning: smooth render mode requested "
		     "with no normals defined.");
        mode &= ~GLM_SMOOTH;
    }
    if (mode & GLM_TEXTURE && !model->texcoords) {
        __glmWarning((char*)"glmDraw() warning: texture render mode requested "
		     "with no texture coordinates defined.");
        mode &= ~GLM_TEXTURE;
    }
    if (mode & GLM_FLAT && mode & GLM_SMOOTH) {
    ...
    if (mode & GLM_COLOR && !model->materials) {
        __glmWarning((char*)"glmDraw() warning: color render mode requested "
		     "with no materials defined.");
        mode &= ~GLM_COLOR;
    }
    if (mode & GLM_MATERIAL && !model->materials) {
        __glmWarning((char*)"glmDraw() warning: material render mode requested "
		     "with no materials defined.");
        mode &= ~GLM_MATERIAL;
    }
    if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
        __glmWarning((char*)"glmDraw() warning: color and material render mode requested "
